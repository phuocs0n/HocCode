# Thought Process

#### Course: Learn Recursion with Python

## Thought Process

Recursive Call

To convert 12 to binary, let's first think what argument needs to be passed in each recursive call.

At first, we will pass 12 as an argument to a function. Then, we will perform integer division on it by 2 to get the quotient (6) and find the remainder as well.

We will then call the function again with the quotient (6) as the parameter. Again, we will divide the function by 2 to get the quotient and the remainder.

Base Condition

The above process will go on recursively till the quotient becomes 0, which is our base condition.

We get the result in binary by concatenating the remainders of each division from bottom to top (or from the last remainder obtained to the first).

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


