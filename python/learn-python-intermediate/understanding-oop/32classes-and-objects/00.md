# Creating a Class

#### Course: Learn Python Intermediate

## Creating a Class

As mentioned before, we need to create a class first before we can create objects from it.

```python
# create a class
class Student:
    pass
```

Here, we have created a class named Student.

For now, we have put a pass statement inside it because the body of a class cannot be empty. We'll gradually add methods (functions) and attributes (variables) inside it.

![Idea Emoji](data:image/png;base64,)

But first, let's create objects from this class.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


