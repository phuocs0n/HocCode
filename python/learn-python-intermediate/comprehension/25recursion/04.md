# Example: Finite Recursion

#### Course: Learn Python Intermediate

## Example: Finite Recursion

In this example, we will create a program to find the sum of natural numbers using recursion.

```python
# sum of natural numbers n
total = 1 + 2 + ... + n
```

### Source Code

```python
1def calculate_sum(n):
2
3    if n != 0:
4       # recursive call
5        n = n + calculate_sum(n-1)
6    
7    return n
8    
9
10result = calculate_sum(3)
11print(result)
```

Output

```text
6
```

Here, the recursive function call is made only when n is not equal to 0.

To meet this condition, we have decreased the value of n by 1 in each function call using calculate_sum(n-1). When n becomes 0, the recursive calls end.

Next, we'll see how this program works with visual illustration.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


