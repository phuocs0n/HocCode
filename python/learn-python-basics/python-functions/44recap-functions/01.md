# Recap (II)

#### Course: Learn Python Basics

## Recap (II)

#### Arguments and Return Value

You can give inputs to a function, called arguments, and the function's output is called the return value.

```python
1# Define the function
2def calculate_product(a, b):
3    total = a * b
4    return total
5
6# Get inputs
7n1 = int(input("Enter a number: "))
8n2 = int(input("Enter a number: "))
9
10# Call the function with arguments
11result = calculate_product(n1, n2)
12
13print(result)
```

#### Local Variables

Variables defined inside a function are local to that function. This means they cannot be accessed outside the function, making the function independent and reusable.

```python
1def say_hello():
2    greeting = "Hello"
3
4# Error! greeting is not defined
5print(greeting)
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


