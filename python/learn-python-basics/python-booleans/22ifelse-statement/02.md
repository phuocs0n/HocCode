# Example: if Statement

#### Course: Learn Python Basics

## Example: if Statement

Let's take a look at an example:

```python
1# Check eligibility for discount.
2
3total = float(input("Enter the total amount: "))
4
5# Check if total is greater than 100
6if total > 100:
7    print("Congratulations!")
8    print("You are eligible for a discount")
```

This program behaves differently depending on the input.

Sample Output 1

```text
Enter the total amount: 120
Congratulations!
You are eligible for a discount.
```

If the user enters 120, total > 100 evaluates to True. In this case, the statements inside the if block are executed.

Sample Output 2

```text
Enter the total amount: 35
```

If the user enters 35, total > 100 evaluates to False. In this case, the statements inside the if block are skipped and not executed.

![Idea Emoji](data:image/png;base64,)

Next, let's use the visualizer to understand how this program works.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


