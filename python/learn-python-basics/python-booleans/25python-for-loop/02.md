# Access Elements

#### Course: Learn Python Basics

## Access Elements

You can also access individual elements of a sequence during its execution. Consider this example:

```python
1name = "Eva"
2
3for item in name: 
4    # Access item
5    print(item)
```

Output

```text
E
v
a
```

In this program, item stores:

1. "E" in the first iteration.
2. "v" in the second iteration.
3. "a" in the third iteration.

![Idea Emoji](data:image/png;base64,)

Run the visualizer below to understand how this code works.

```python
1name = "Eva"
2
3for item in name: 
4    # Access item
5    print(item)
```

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


