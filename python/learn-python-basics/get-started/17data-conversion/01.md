# Convert to Integer

#### Course: Learn Python Basics

## Convert to Integer

We use the int() function to convert data from other types to integers.

#### Convert Float to Integer

```python
1float_number = 15.0
2
3
4# Convert float to integer
5integer_number = int(float_number)
6
7
8print(integer_number)   # Output: 15
```

Here, the int() function converts the floating-point number to its equivalent integer.

#### String to Integer

```python
1string_number = "15"
2
3
4# Convert string to integer
5# integer_number will contain 15
6integer_number = int(string_number)
7
8
9print(integer_number)   # Output: 15
```

Here, the int() function converts the string to its equivalent integer.

![Idea Emoji](data:image/png;base64,)

Next, we will see an example.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


