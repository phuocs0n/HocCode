# Convert to Float

#### Course: Learn Python Basics

## Convert to Float

We use the float() function to convert data from other types to floating-point numbers.

#### Integer to Float

```python
1integer_number = -15
2
3
4# Convert integer to float
5float_number = float(integer_number)
6
7
8print(float_number)   # Output: -15.0
```

Here, the float() function converts the integer number to its equivalent floating-point number.

#### String to Float

```python
1string_number = "-15.65"
2
3
4# Convert string to float
5float_number = float(string_number)
6
7
8print(float_number)   # Output: -15.65
```

Here, the float() function converts the string to its equivalent floating-point number.

![Idea Emoji](data:image/png;base64,)

Next, we will see another example of an incompatible conversion.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


