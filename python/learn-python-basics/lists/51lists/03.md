# List Length

#### Course: Learn Python Basics

## List Length

As we have seen in the previous chapter, we can use the built-in len() function to get the number of items a list has.

The len() function takes a list as an argument and returns an integer (length of the list).

```python
1# list containing 3 items
2languages = ['Python', 'JavaScript', 'C++']
3
4length = len(languages)
5print(length)    # Output: 3
6
7# empty list
8animals = []
9
10length = len(animals)
11print(length)   # Output: 0
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


