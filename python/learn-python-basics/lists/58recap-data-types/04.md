# Example: Employee in Both Departments

#### Course: Learn Python Basics

## Example: Employee in Both Departments

Suppose, we have two lists:

- frontend_developers - a list of frontend developers of the company
- backend_developers - a list of backend developers of the company

Now, if we need to find a list of developers who can work as both frontend and backend, here's how we can do it.

```python
1frontend_developers = ['Rob', 'Jane', 'Mary', 'Anne']
2backend_developers = ['Jane', 'Jack', 'Lily']
3
4# convert lists to sets and perform set intersection
5both_developers = set(frontend_developers) & set(backend_developers)
6
7# convert to list and print it
8print(list(both_developers))
```

Output

```text
['Jane']
```

In the program, we have converted frontend_developers and backend_developers to sets and performed set intersection.

```python
both_developers = set(frontend_developers) & set(backend_developers)
```

This gives a set of developers who are common to both frontend and backend.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


