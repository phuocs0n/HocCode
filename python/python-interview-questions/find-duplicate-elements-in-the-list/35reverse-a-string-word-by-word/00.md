# Question

#### Course: Python Interview Questions

## Question

Given a string of length n, write a program to reverse the given string word by word.

For example,

If you are given a string,

```python
this is blue
```

Then, by reversing words of the string, we get,

```python
blue is this
```

Similarly, the reverse of the string 'welcome to hollywood' is 'hollywood to welcome'.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


