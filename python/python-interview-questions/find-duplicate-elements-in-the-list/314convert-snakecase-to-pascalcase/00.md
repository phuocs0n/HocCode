# Question

#### Course: Python Interview Questions

## Question

An intern at a company followed the snake_case convention while writing codes. But the company has been using PascalCase for a long time.

Now, write a Python program that converts snake_case strings to PascalCase.

For example,

If the given string is,

```python
custom_date_builder
```

Your program should print,

```python
CustomDateBuilder
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


