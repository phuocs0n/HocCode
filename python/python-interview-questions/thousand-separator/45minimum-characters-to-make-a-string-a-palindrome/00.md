# Question

#### Course: Python Interview Questions

## Question

Given a non-empty string name, find the minimum number of characters needed to make name a palindrome.

A palindrome string is a string that reads the same backwards as well as forwards. For example, php, madam, etc.

Suppose, the value stored in name is 'joe'. In order to make the string name a palindrome, we need to add two characters 'e' and 'o' (2 characters) before 'joe'. This gives a string 'eojoe', which is a palindrome.

For example

If the given string is

```python
xyz
```

your program should give 2 as the output.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


