# Question

#### Course: Python Interview Questions

## Question

Suppose we have an integer list where all elements of the list represent a number. Find the resulting value by adding 1 to the number.

For example

```python
digits = [1, 4, 2, 4]
```

Here, all elements of the list represent a number, 1424. Now, we need to add 1 to the number. Hence, our output will be

```python
[1, 4, 2, 5]
```

which is equivalent to 1424 + 1.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


