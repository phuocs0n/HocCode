# Question

#### Course: Python Interview Questions

## Question

You are given a string named mixed_string, which consists of lowercase alphabets and integers from 0-9.

Arrange mixed_string in such a way that

- the alphabets sorted in alphabetic order come first,
- which is then followed by the sum of all the integers.

Let's take a look at an example to better illustrate the question.

Suppose you are given a string,

```python
mixed_string = '9python123'
```

Here, you need to arrange 'python' in alphabetical order followed by the sum of integers present in the string.

Since mixed_string contains the integers 9, 1, 2, and 3, the sum of integers is 9 + 1 + 2 + 3 = 15.

So the result will be,

```python
hnopty15
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


