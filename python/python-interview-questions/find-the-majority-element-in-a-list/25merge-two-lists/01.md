# Thought Process

#### Course: Python Interview Questions

## Thought Process

Python allows us to merge two lists by joining them using the + operator. Here's the syntax to merge lists:

```python
merged_list = fruits + vegetables
```

The merged_list here will include all elements of fruits and vegetables.

We can also merge lists of different data types using this method. For example,

```python
fruits = ['apple', 'orange', 'banana']
numbers = [5, 8, 10]

merged_list = fruits + numbers
```

In this case, merged_list will include all elements of fruits and numbers.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


