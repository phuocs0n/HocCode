# Question

#### Course: Python Interview Questions

## Question

Given a non-empty string, find all the vowels present in the string and return a string without the vowels.

In English alphabets, vowels are 'a', 'e', 'i', 'o', and 'u' along with their uppercase forms 'A', 'E', 'I', 'O', 'U'.

Suppose we need to remove all the vowels from the string 'water'. Here, the vowels are 'a' and 'e'. So the resulting string without the vowels is 'wtr'.

Remember that strings are immutable, so we can not change the original string. So initialize a new string that stores the resulting string (string without vowels).

For example,

If the given string is

```python
'icecream23'
```

Then the output should be

```python
'ccrm23'
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


