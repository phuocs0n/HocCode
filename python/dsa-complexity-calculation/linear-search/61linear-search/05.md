...
```text
=sum of all possible casesno. of cases= \cfrac{sum\ of\ all\ possible\ cases}{ no.\ of\ cases}=no. of casessum of all possible cases​=1+2+3+...+nn= \cfrac{1+2+3+...+n}{n}=n1+2+3+...+n​=n⋅(n+1)2n(using the formula for the sum of first n natural numbers)= \cfrac{n \cdot \cfrac{(n + 1)}{2}}{n} \quad \text{(using the formula for the sum of first n natural numbers)}=nn⋅2(n+1)​​(using the formula for the sum of first n natural numbers)=O(n)(using frequency count method)= O(n) (using \ frequency \ count \ method)=O(n)(using frequency count method)
```
