# Creating an Array

#### Course: Learn C Programming

## Creating an Array

An array is created by placing elements inside curly braces {}, separated by commas. For example:

```c
int marks[10] = {70, 90, 67, 32, 88, 45, 53, 0, 90, 54};
```

Here,

- marks - name of the array
- [10] - represents the size of the array (number of elements)
- {70, 90, 67, 32, 88, 45, 53, 0, 90, 54} - elements of the array
- int - represents that data type of elements array can store.

![Idea Emoji](data:image/png;base64,)

After a quiz, we'll learn how to access array elements.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


