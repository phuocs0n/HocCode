# Thought Process

#### Course: Learn Recursion With Java

## Thought Process

Recursive Call

To convert 6 to binary, let's first think what argument needs to be passed in each recursive call.

1. At first, we will pass 6 as an argument to the method.
2. Then, we will perform integer division (6 / 2) to get the quotient (3) and find the remainder as well.
3. We will then call the method again with the quotient (3) as the parameter.
4. Again, we will divide the method by 2 to get the quotient and the remainder.

Base Condition

The above process will go on recursively till the quotient becomes 0, which is our base condition.

We get the result in binary by concatenating the remainder of each division from bottom to top (or from the last remainder obtained to the first).

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


