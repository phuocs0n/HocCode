# Create HashSet

#### Course: Learn Java Intermediate

## Create HashSet

The HashSet class is defined inside the HashSet package, so to use it we first have to import the package.

```java
import java.util.HashSet;
```

Once we import the package, we can create an object of the HashSet class.

```java
HashSet<String> animals = new HashSet<>();
```

Here, we have created a String type hashset named animals.

Now we can use the add() method to store unique elements in animals. Let's see an example,

```java
1import java.util.HashSet;
2
3class Main {
4    public static void main(String[] args) {
5
6        // create HashSet 
7        HashSet<String> animals = new HashSet<>();
8
9        // add elements to HashSet
10        animals.add("tiger");
11        animals.add("dog");
12        animals.add("elephant");
13
14        // print HashSet
15        System.out.println(animals);
16    }
17}
```

Output

```text
[tiger, dog, elephant]
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


