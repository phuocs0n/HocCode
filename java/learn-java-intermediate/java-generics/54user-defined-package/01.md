# Importing User-defined package

#### Course: Learn Java Intermediate

## Importing User-defined package

In the previous example, we created a package and created two java files inside of it.

```java
package programiz;

public class Teacher {
    
    public void getTeacherInfo(int id, String name) {
      
        System.out.println(id);
        System.out.println(name);
    }
}
```

```java
package programiz;

public class Student {
    
    public void getStudentInfo(int id, String name) {
        
        System.out.println(id);
        System.out.println(name);
    }
}
```

Now, to use the methods fields of the Teacher.java and Student.java in another java file (outside of the package) we must import the Student and Teacher class from the programiz package.

```java
import programiz.Teacher;
import programiz.Student;

class Main {
    
    public static void main(String[] args) {
        
        Teacher t1 = new Teacher();
        System.out.println("Teacher info");
        t1.getTeacherInfo(1, "Bruce");
        
        Student s1 = new Student();
        System.out.println("Student info");
        s1.getStudentInfo(11, "Peter");
    }
}
```

Output

```text
Teacher info
1
Bruce
Student info
11
Peter
```

In the above program,

```java
import programiz.Teacher;
```

imports Teacher class of programiz package to Main class. Now, we can create objects and use the methods, and fields of the Teacher class in the Main class.

```java
Teacher t1 = new Teacher();
System.out.println("Teacher info");
t1.getTeacherInfo(1, "Bruce");
```

Similarly,

```java
import programiz.Student;
```

imports Student class of programiz package to Main class. Now, we can create objects and use the methods, and fields of the Student class in the Main class.

```java
Student s1 = new Student();
System.out.println("Student info"); 
s1.getStudentInfo(11, "Peter");
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


