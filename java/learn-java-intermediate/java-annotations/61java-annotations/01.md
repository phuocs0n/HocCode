# @Override Annotation

#### Course: Learn Java Intermediate

## @Override Annotation

During inheritance, we know that if the same method is present in both the parent class and the child class, the method in the child class overrides the method of the parent class.

If we want to specify the method overriding, we use the @Override annotation. Let's see an example,

```java
1class Animal {
2    public void displayInfo() {
3        System.out.println("I am an animal.");
4    }
5}
6
7class Dog extends Animal {
8
9    @Override
10    public void displayInfo() {
11        System.out.println("I am a dog.");
12    }
13}
14
15class Main {
16    public static void main(String[] args) {
17        Dog d1 = new Dog();
18        d1.displayInfo();
19    }
20}
```

Output

```text
I am a dog.
```

Here, we are overriding the displayInfo() method of the Animal class in the Dog class.

The @Override annotation is used above displayInfo() of Dog to specify that it is the overridden method.

Once we use @Override, the method should be overridden from the parent class. Otherwise, we will get an error.

```java
1class Animal {
2
3}
4
5class Dog extends Animal {
6
7    @Override
8    public void displayinfo() {
9        System.out.println("I am a dog.");
10    }
11}
12
13class Main {
14    public static void main(String[] args) {
15        Dog d1 = new Dog();
16        d1.displayinfo();
17    }
18}
```

Here, there is no displayInfo() in the parent class, so method overriding doesn't take place. Hence, we get an error

```text
method does not override or implement a method from a supertype
    @Override
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


