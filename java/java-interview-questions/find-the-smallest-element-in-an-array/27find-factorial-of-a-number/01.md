# Thought Process

#### Course: Java Interview Questions

## Thought Process

To solve this problem, we need to find all positive numbers less than the number and then multiply those numbers with the number.

Let's see a detailed overview of how we can solve this problem.

- Create a factorial variable and assign 1 to it.
- Use a loop to iterate from number to 1. On each iteration of the loop, multiply the result variable by the current loop value.
- After the loop ends, return the factorial variable.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


