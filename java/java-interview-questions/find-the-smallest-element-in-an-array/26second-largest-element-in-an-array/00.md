# Question

#### Course: Java Interview Questions

## Question

Write a Java program to find the second largest element in an array. Here, the array could be of any size and the elements could be in any order. For example,

```java
numbers = {2, 3, 5, 6, 1}
output = 5
```

```java
numbers = {2, 33, 35, 36, 11}
output = 35
```

```java
numbers = {1, 23, 25, 26, 21}
output = 25
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


