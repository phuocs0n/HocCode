# Question

#### Course: Java Interview Questions

## Question

Given a non-empty string. Find the unique characters in the given string.

Unique characters are those characters that exist only one time in the string. For example,

```java
text = programiz
output = [p, o, g, a, m, i, z]
```

```java
text = computer
output = [c, o, m, p, u, t, e, r]
```

```java
text = giggle
output = [i, l, e]
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


