# Creating a Class

#### Course: Learn Java OOP

## Creating a Class

As mentioned before, we need to create a class before creating objects from it.

In Java, we use the class keyword to define a class. Remember this code from the Java Basics Course.

```java
class Main {
    ...
}
```

We have used this in all of our programs. Here, Main is a class.

![Idea Emoji](data:image/png;base64,)

We can use any name for our class. For example,

```java
class Student {
    ...
}
```

Here, we have created a class named Student.

A class can contain:

- fields (variables) - to store data
- methods - to perform tasks on fields

We will gradually add methods and fields inside a class. But first, let's create objects from the class.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


