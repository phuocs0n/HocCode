# Float Type Variables

#### Course: Learn Java Basics

## Float Type Variables

So far we have learned about:

- int variable to store numbers without decimal parts
- double variable to store floating-point numbers
- char variables to store single characters
- String variables to store textual data

```java
int age = 25;
double salary = 52623.34;
char alphabet = 'z';
String name = "Programiz";
```

Similar to double, the float type variable is used to store floating-point numbers (decimal numbers). For example,

```java
1class Main {
2    public static void main(String[] args) {
3        
4        float number = 54.76f;
5        System.out.println(number);
6        
7    }
8}
```

Output

```text
54.76
```

Here, the f after the value denotes it is of float type instead of double. We use either f or F to denote float.

If both float and double store floating-point values, then what's the difference between them?

The major difference between double and float is the size. The size of the float type is usually 4 bytes and the size of double is 8 bytes.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


