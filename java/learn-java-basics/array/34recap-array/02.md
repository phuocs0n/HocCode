# Recap (II)

#### Course: Learn Java Basics

## Recap (II)

for each Loop

The for-each loop makes accessing array elements so easy.

```java
1class Main {
2    public static void main(String[] args) {
3        
4        // Create an array
5        int[] primeNumbers = {2, 3, 5, 7};
6        
7        System.out.println("Prime Numbers:");
8        
9        for (int number : primeNumbers) {
10            System.out.println(number);
11        }
12    }
13}
```

Here, in each iteration of the for-each loop, an element of the primeNumbers array is stored in the number variable. This is why the array elements are printed when we print number. Now let us look at some examples that involve arrays.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


