# Introduction to Course

#### Course: Game Logic with JavaScript

## Introduction to Course

Hi there! 👋

Welcome to Thinking in Games — the first course on the JavaScript Game skill track. We're glad you're here.

In this course you will:

- think like a game developer
- learn by building one real game, step by step
- see every concept tie directly to that goal
- see how each idea fits into a working quiz game (no memorizing definitions in isolation)

We focus on one thing: building that game together. By the end, you will have a mini quiz you can run, replay, and call your own.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


