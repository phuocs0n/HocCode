# Your first array

#### Course: Game Logic with JavaScript

## Your first array

We create an array with square brackets. Items are separated by commas. We access one item with the index in square brackets (0 for first, 1 for second):

```javascript
1var options = ["A: Python", "B: Pizza", "C: Pencil"];
2
3console.log(options[0]);
4console.log(options[1]);
5console.log("Length: " + options.length);
```

Here, options holds three strings. options[0] is the first, options[1] is the second. options.length is how many items are in the array. In the quiz we'll use an array of options for each question and an array of question objects.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


