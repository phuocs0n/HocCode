# Console

#### Course: JavaScript in Browser

## Console

Let's display a simple text using console.log().

```html
1<header id="header">
2    <div id="logo">E-Commerce</div>
3    ...
```

```css
1body {
2    margin: 0;
3    ...
```

```javascript
1console.log("E-commerce site loaded!");
```

Run the above code.

You'll see the logged text in the Console tab. But if you switch to the Browser tab, you'll notice the webpage is still displayed.

Now that you're familiar with our code editor, let's get started.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


