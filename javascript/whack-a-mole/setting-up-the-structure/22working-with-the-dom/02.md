# The main() Function Pattern 📦

#### Course: Whack A Mole

## The main() Function Pattern 📦

From this point forward, you'll wrap all your game code inside a main() function. This is a common pattern in JavaScript development.

Here's what it looks like:

```javascriptDOM
function main() {
    // All your game code goes here
    console.log('Game is starting!');
}

main();
```

#### Why use a main() function?

- Organization: All your code lives in one clear place
- Scope control: Variables inside main() don't pollute the global space
- Professional pattern: Real projects use this approach to structure code

![Idea Emoji](data:image/png;base64,)

In the next section, you'll create your first main() function. Let's get started!

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


