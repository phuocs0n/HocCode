# What Just Happened

#### Course: API Handling with Asynchronous Javascript

## What Just Happened

When you clicked the Load Articles button, JavaScript started executing the fetchArticlesBlocking() function. It does nothing other than checking the current time for 3 seconds.

During those 3 seconds, JavaScript can't do anything else since it's stuck in that loop. It can't:

- Update the UI
- Respond to clicks
- Handle scroll events
- Run any other code

This is what "blocking" means: our code blocks everything else from happening.

In this simulation, I blocked the page using a while loop.

In reality, blocking happens when you:

- make a network request using old synchronous methods,
- or perform heavy calculations,
- or read large files.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


