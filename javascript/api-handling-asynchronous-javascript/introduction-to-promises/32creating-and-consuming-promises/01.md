# Creating a Promise

#### Course: API Handling with Asynchronous Javascript

## Creating a Promise

When you create a Promise, you are normally telling Javascript:

"I'm starting some work now, and I'll tell you later whether it succeeded or failed.

So when you write:

```javascript
const promise = new Promise((resolve, reject) => {
    // Asynchronous work
});
```

Javascript immediately runs that function and hands you two callbacks:

- resolve(value) → means success
- reject(error) → means failure

![Idea Emoji](data:image/png;base64,)

Only one of these can happen.

Once a promise is resolved, it can't be rejected later (and vice versa).

This makes promises reliable — you'll never get both success and failure from the same promise.

Now, you might be wondering what happens when you call resolve() or reject(). It's just you are not ending the Promise. You are passing the result forward.

You will learn about it in detail in the coming lessons.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


