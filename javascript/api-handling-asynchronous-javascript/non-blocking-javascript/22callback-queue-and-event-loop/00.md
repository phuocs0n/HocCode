# The Callback Queue

#### Course: API Handling with Asynchronous Javascript

## The Callback Queue

To understand why the callback runs later, we need to learn a new concept: the callback queue.

Basically, the callback queue is a waiting area for callback functions.

After the Web API completes its asynchronous tasks, it ensures that they don't run immediately. Instead, it pushes the callback to the callback queue.

![Idea Emoji](data:image/png;base64,)

Click Next Lesson to see how this queue works.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


