# How await Works

#### Course: API Handling with Asynchronous Javascript

## How await Works

As mentioned before, the await keyword pauses function execution until the promise resolves. For example,

```javascript
1async function fetchUserData() {
2  console.log('Starting fetch...');
3  
4  // Simulate an API call by using a delay and returning mock data
5  const mockData = {
6    name: "Leanne Graham",
7    username: "Bret",
8    email: "Sincere@april.biz"
9  };
10
11  const user = await new Promise(resolve => 
12    setTimeout(() => resolve(mockData), 1000)  // Simulate network delay and resolve with mock data
13  );
14
15  console.log('Fetch complete, parsing JSON...');
16  console.log('User data ready:', user.name);
17
18  return user;
19}
20
21fetchUserData();
```

When JavaScript hits the first await, it:

- Pauses the function at that line.
- Waits for the promise to resolve.
- Resumes execution with the resolved value.
- Continues to the next line.

![Idea Emoji](data:image/png;base64,)

Obviously, await pauses your function without pausing JavaScript — everything else still runs. The UI stays responsive. Events still fire.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


