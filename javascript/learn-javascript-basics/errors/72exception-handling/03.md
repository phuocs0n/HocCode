# Example: The try…catch Statement

#### Course: Learn JavaScript Basics

## Example: The try…catch Statement

Consider this code:

```javascript
1try {
2    // Access non-existing variable
3    console.log(result);
4
5} catch (error) {
6
7    // If an exception occurs, this will be executed
8    console.log(`Error caught: ${error}`);
9}
10
11console.log("End of program");
```

Output

```text
Error caught: ReferenceError: result is not defined
End of program
```

In this case, the try block attempts to display the result variable. However, since result hasn't been defined, it does not exist.

As a result, an exception occurs, which is caught by the catch block and the error message is displayed.

Instead of halting the program, the exception is handled, and the program continues to run, logging "End of program".

Next, you'll learn the error variable that you see in the catch block.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


