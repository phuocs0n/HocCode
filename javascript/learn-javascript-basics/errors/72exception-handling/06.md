# Example: try...catch...finally with Error

#### Course: Learn JavaScript Basics

## Example: try...catch...finally with Error

Consider this code:

```javascript
1try {
2    console.log(result);
3
4} catch (error) {
5    console.log(`Error caught: ${error}`);
6
7} finally {
8
9    // Code that runs regardless of error
10    console.log("Finally executed");
11}
```

Output

```text
Error caught: ReferenceError: result is not defined
Finally executed
```

In this example, the finally block executes after the exception is caught in the catch block.

![Idea Emoji](data:image/png;base64,)

Now, let's see how finally behaves when no exception occurs.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


