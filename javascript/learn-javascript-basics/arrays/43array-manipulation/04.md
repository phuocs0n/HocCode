# Using The map() Method

#### Course: Learn JavaScript Basics

## Using The map() Method

The syntax for the map() method is:

```javascript
array.map(function)
```

Here,

- array: The array you want to transform.
- function: A function that performs an action on each element of the array.

Example

Suppose you want to increase the prices in an array by $20. Previously, we solved this using a for...of loop. Now, let's use map().

First, define a function to perform the transformation:

```javascript
// Function to increase price by $20
const increasePrice = (price) => {
    return price + 20;
};
```

This function takes a price and returns the updated value.

Now, use this function with map():

```javascript
const newPrices = prices.map(increasePrice);
```

The map() method creates a new array with the updated prices, and stores in newPrices.

![Idea Emoji](data:image/png;base64,)

Let's see the complete code on the next page.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


