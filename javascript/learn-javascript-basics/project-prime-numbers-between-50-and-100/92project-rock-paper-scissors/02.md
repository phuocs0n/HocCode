# Project Break Down

#### Course: Learn JavaScript Basics

## Project Break Down

Here's how our project works:

- The game is played between the computer and the user.
- The computer will randomly choose rock, paper, or scissors. The user won't know the computer's choice.
- The user will then be prompted to choose rock, paper, or scissors.
- We'll compare the computer's choice and the user's choice. If they are the same, it's a draw. Otherwise, we'll determine the winner based on the winning rule.

We'll build the project in the following order:

Step 1: Generate the computer's choice: rock, paper, or scissors.

Step 2: Take the user's input: rock, paper, or scissors.

Step 3: Decide the winner using the defined winning rules.

Step 4: Handle invalid user input to ensure correct choices are entered.

Let's get started.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


