# Understanding Comments

#### Course: Learn JavaScript Basics

## Understanding Comments

Let's look at another example.

```javascript
1// Display Hello
2console.log("Hello");    // Output: Hello
```

Here, the program contains two comments:

- // Display Hello
- // Output: Hello

JavaScript ignores both of these comments. So, this program is also equivalent to:

```javascript
1console.log("Hello");
```

![Idea Emoji](data:image/png;base64,)

Now, let's reinforce your understanding with some quizzes.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


