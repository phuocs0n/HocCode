# Example: Book Object

#### Course: Learn JavaScript Basics

## Example: Book Object

Here's the book object from the introduction page:

```javascript
1const book = {
2    title: "The Hobbit",
3    author: "J.R.R. Tolkien",
4    pages: 295,
5    year: 1937
6};
7
8console.log(book);
```

Output

```text
{
  title: 'The Hobbit',
  author: 'J.R.R. Tolkien',
  pages: 295,
  year: 1937
}
```

In this object, each piece of information is stored as a key-value pair:

- The title key stores the value "The Hobbit".
- The author key stores the value "J.R.R. Tolkien".
- The pages key stores the value 295.
- The year key stores the value 1937.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


