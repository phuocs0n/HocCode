# The slice() Method

#### Course: Learn JavaScript Basics

## The slice() Method

The slice() method is used to extract a part of a string. It creates a new string without changing the original one. For example,

```javascript
1let movie = "Snowpiercer";
2
3let slicedText = movie.slice(0, 4);
4
5console.log(slicedText);    // Output: Snow
```

Here, movie.slice(0, 4) takes a portion of the string from index 0 to 4 (but doesn't include the character at index 4). So, the result is "Snow".

Thus, the substring includes characters at positions 0, 1, 2, and 3 (but not 4).

The second parameter in the slice() method is optional. Let's see what happens when you omit it.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


