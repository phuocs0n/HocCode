# Introduction

#### Course: Learn JavaScript Intermediate

## Introduction

JSON stands for JavaScript Object Notation. It is a lightweight data format used to store and transfer data usually between web servers and web pages.

Lets see an example of JSON data.

```javascript
let person = {
    "name": "Beyoncé,
    "age": 42,
    "gender": "female"
}
```

JSON objects are written inside curly braces { }. They contain multiple key-value pairs separated by a comma ,.

Although the JSON syntax resembles JavaScript object literal syntax, they are not the same. You will learn about their differences in the lessons later.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


