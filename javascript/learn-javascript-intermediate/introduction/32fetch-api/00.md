# Introduction

#### Course: Learn JavaScript Intermediate

## Introduction

As you begin doing JavaScript projects, you'll often need to get data from the internet. To do this, you will come across the concept of the fetch() function.

fetch() is a built-in JavaScript function used to make requests to servers. It allows you to retrieve data from a server and handle the response.

![Idea Emoji](data:image/png;base64,)

This course will skim through the concept and usage of fetch(). We will get into specifics in future JavaScript courses.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


