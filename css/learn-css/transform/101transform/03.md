# Translate to the Left or Top

#### Course: Learn CSS

## Translate to the Left or Top

Just like you can move an element to the right or down using positive values, you can also use negative values to move it to the left or above.

Let's try this out. This time, we'll translate box 50px to the left and 100px above.

```html
1<h1>Translate 50px to the left and 100px above.</h1>
2<p>Click the box to see the change.</p>
3<div class="box">Click Me!</div>
```

```css
1.box:active {
2    /* Moves box 50px to the left and 100px above */
3    translate: -50px -100px;
4}
```

Run the above code and click the box element to see how it moves 50px to the left and 100px above its original position.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


