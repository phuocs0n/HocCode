# Numeric Font Weight

#### Course: Learn CSS

## Numeric Font Weight

Numeric values (100, 200, ..., 1000) are also common for font weight. Smaller values make the font thinner, while larger values make it thicker.

```html
1<h2>Web development</h2>
2
3<p>Web development is fun!</p>
4
5<ul>
6    <li>HTML for structure.</li>
7    <li>CSS for styling.</li>
8    <li>JavaScript for interactivity.</li>
9</ul>
```

```css
1/* The font-weight of paragraphs is set to 500. */
2p {
3    font-weight: 500;
4}
5
6/* The font-weight of unordered lists is set to 600. */
7ul {
8    font-weight: 600;
9}
```

If you look at the output, the font weight of the paragraph is unchanged. This is because the default font (Times New Roman) doesn't support the specified 500 weight.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


