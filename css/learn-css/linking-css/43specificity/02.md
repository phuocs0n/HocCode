# Example 2: ID Selectors

#### Course: Learn CSS

## Example 2: ID Selectors

ID selectors are even more specific, targeting elements with a specific ID attribute. So, they override both class and element selectors.

```html
1<p id="my-id" class="my-class">This is a paragraph.</p>
```

```css
1/* Element selector */
2p {
3    color: red;
4}
5
6/* Class selector */
7.my-class {
8    color: green;
9}
10
11/* ID selector */
12#my-id {
13    color: blue;
14}
```

In this example, the ID selector #my-id overrides both the class and element selectors, so the paragraph text will be blue.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


