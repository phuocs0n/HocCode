# Use Consistent Naming Conventions II

#### Course: Learn CSS

## Use Consistent Naming Conventions II

ii. Use a Consistent Naming Convention

Stick to a consistent naming convention, like kebab-case (e.g., .main-header), for better readability.

Good Example

```css
/* kebab-case naming convention */

.header-logo {
    width: 100px;
}

.button-primary {
    background-color: #3498db;
    color: white;
}

.text-secondary-red {
    color: red;
    font-size: #14px;
}
```

In this example, the consistent use of kebab-case makes the code clear and easy to read.

Bad Example

```css
/* Inconsistent naming */

/* Use of camelCase */
.headerLogo {
    width: 100px;
}

/* Use of kebab-case */
.btn-primary {
    background-color: #3498db;
    color: white;
}

/* Use of snake_case */
.text_secondary_red {
    color: red;
    font-size: #14px;
}
```

In the bad example, the mix of camelCase, kebab-case, and snake_case may cause confusion and errors.

Following clear, consistent naming conventions ensures your CSS is organized, easy to understand, and scalable as your project grows.

![Idea Emoji](data:image/png;base64,)

As you work on more projects, gradually trying out BEM can be a great next step. For beginners, we're using kebab-case here to keep it simple and consistent.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


