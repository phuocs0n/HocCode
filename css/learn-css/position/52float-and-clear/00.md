# Introduction

#### Course: Learn CSS

## Introduction

The float property allows you to push an element to the left or right and lets the rest of the content flow around it.

This technique was commonly used for creating two-column layouts, where elements are placed side by side.

![Idea Emoji](data:image/png;base64,)

This chapter covers the basics of float and clear, providing just enough information to understand and work with legacy code. Let's start with some examples.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


