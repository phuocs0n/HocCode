# Example: justify-content

#### Course: Learn CSS

## Example: justify-content

Try different values for justify-content by uncommenting each commented line one-by-one and see how it changes the arrangement:

```html
1<div class="container">
2    <div class="box box1">First Item</div>
3    <div class="box box2">Second Item</div>
4    <div class="box box3">Third Item</div>
5</div>
```

```css
1.container {
2    display: flex;
3
4    /* Uncomment one of the following to test different justify-content values */
5
6    /*     justify-content: flex-end;      */
7    /*     justify-content: flex-start;    */
8    /*     justify-content: center;        */
9    /*     justify-content: space-between; */
10    /*     justify-content: space-around;  */
11    /*     justify-content: space-evenly;  */
12}
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


