# Inline Vs. Block Elements

#### Course: Learn HTML

## Inline Vs. Block Elements

Consider this code:

```html
1<h1>Heading</h1>
2<p>A paragraph</p>
```

If you run the code, you can see these two tags are displayed on separate lines by default.

Now, let's run this code:

```html
1<strong>Strong</strong>
2<strong>Even stronger</strong>
```

Here, both tags are displayed on the same line.

The reason is that tags such as <h1>, <p> and <ul> are block-level elements by default. Meaning they take the whole block.

In contrast, <strong> is an inline element that doesn't extend the whole block.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


