# Embed Image from URL

#### Course: Learn HTML

## Embed Image from URL

It is also possible to embed images from an absolute URL. For example,

```html
1<img src="  
2https://blog-cms.programiz.pro/assets/90492a88-aba4-4368-87b6-dbcf4339c9b0
3">
```

The image from this link is displayed on the webpage.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


