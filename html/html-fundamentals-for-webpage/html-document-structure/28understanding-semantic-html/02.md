# A Question Developers Eventually Ask

#### Course: HTML Fundamentals for Your First Webpage

## A Question Developers Eventually Ask

At this point, you've written a lot of HTML. But you might be wondering:

"Why does HTML have so many tags that seem to do the same thing?"

1. <div> vs <section>
2. <section> vs <article>
3. <strong> vs just text
4. Headings vs paragraphs

The answer is meaning — they may appear similar visually, but the way the web interprets them is different.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


