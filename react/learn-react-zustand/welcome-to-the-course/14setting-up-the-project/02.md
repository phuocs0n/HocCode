# The File Structure

#### Course: Learn React Zustand: The Smarter Way to Handle Data

## The File Structure

Here is how we will organize the project:

```react
src/
├── store/
│   └── useJournalStore.js
├── components/
│   ├── Header.jsx
│   ├── JournalForm.jsx
│   ├── EntryCard.jsx
│   └── EntryList.jsx
├── App.jsx
└── index.jsx
```

useJournalStore.js is the most important file in this course. That is where your Zustand store will live. Right now it is empty. By the end of the course it will hold all the state for the entire app.

Everything else starts as an empty shell. You will fill them in one lesson at a time.

![Idea Emoji](data:image/png;base64,)

Next, you will see how Zustand is installed. Click Next Lesson to continue.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


