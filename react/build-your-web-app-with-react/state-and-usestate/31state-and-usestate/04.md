# Introducing useState

#### Course: Build your First Web App with React

## Introducing useState

React gives you a hook called useState to add state to a component.

![Idea Emoji](data:image/png;base64,)

You call it with the initial value of the state. It returns two things: the current value, and a function to update it.

An example is given below:

```react
const [entries, setEntries] = useState([]);
```

Here:

- entries is the current value (an array). We start with an empty array [] as the initial value, which is returned to entries.
- setEntries is the function you will call when you want to change the array (for example when adding an entry).

Important Note:

You must call useState at the top level of your component, not inside a loop or an if statement.

That's because React uses the order of hooks to keep the state correct. You'll learn more about this in a future course.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


