# Conditional Rendering in JSX

#### Course: Build your First Web App with React

## Conditional Rendering in JSX

You can use JavaScript logic inside JSX with {}. Thus, you can use an if or a ternary, or a short-circuit like condition && <Something />.

Here are some examples:

1. Ternary Expression

```react
{entries.length === 0 ? <p>No entries yet.</p> : <List />}
```

2. Short-Circuit

You can render the list separately after running this code:

```react
{entries.length === 0 && <p>No entries yet.</p>}
```

3. Code Before the return Statement

You can compute the content in a variable before using return, and then use the variable in your JSX.

![Idea Emoji](data:image/png;base64,)

Next, we'll learn how to display a message when the list is empty.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


