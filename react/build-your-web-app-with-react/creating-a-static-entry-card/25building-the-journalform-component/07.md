# What's a Fragment?

#### Course: Build your First Web App with React

## What's a Fragment?

Right now, your components wrap everything inside a <div>. That works just fine, but sometimes you don't want an extra <div> in your HTML.

React lets you use a Fragment, which is an invisible wrapper. It groups elements without adding a DOM node.

Instead of writing:

```react
return (
  <div>
    <input />
    <textarea />
    <button />
  </div>
);
```

You can write:

```react
return (
  <>
    <input />
    <textarea />
    <button />
  </>
);
```

![Idea Emoji](data:image/png;base64,)

The <> and </> are shorthand for <React.Fragment> and </React.Fragment>. They group the elements, but they don't render a <div> in the HTML.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


