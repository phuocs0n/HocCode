# The JSX Syntax

#### Course: Build your First Web App with React

## The JSX Syntax

As you might have already guessed, the basic syntax to create a JSX element is:

```react
function ReactFunction() {
  return (
    {/* JSX code */}
  );
}
```

Notice that the JSX code is part of a React function. We don't usually write JSX in the open; instead, we return the JSX to a function.

![Idea Emoji](data:image/png;base64,)

However, there's one rule you must follow when writing JSX. Let's explore that next.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


