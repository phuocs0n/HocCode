# When Do You Need React?

#### Course: Build your First Web App with React

## When Do You Need React?

Before proceeding further, I have a word of caution for you:

Don't use React just because it's popular!

For a simple dropdown or a small form, plain JavaScript is faster as it requires no setup and takes up fewer lines of code.

Only use React when:

- Your UI has lots of states that change often. For example, a social media feed where posts load dynamically, "likes" update in real time, and comments appear without refreshing the page.
- One action must update many parts of the page. For instance, when you add a filter that affects several charts and a table.

In plain JS, you'd manually update each spot and risk missing one; in React, you update the data once and the UI stays in sync.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


