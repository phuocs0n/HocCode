# When to Use useCallback()

#### Course: React Hooks: Level Up Your First Web App

## When to Use useCallback()

Not every function needs to be wrapped. Here's a quick reference:

| Situation | Worth wrapping? |
| --- | --- |
| Function is in a useEffect() dependency array (directly or via prop) | Yes |
| Function is passed as a prop to a child that checks for prop identity | Yes |
| Function is used only as an onClick or similar event handler | No |

When in doubt, check whether any useEffect() in the receiving component lists the function as a dependency. If it does, wrap it.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


