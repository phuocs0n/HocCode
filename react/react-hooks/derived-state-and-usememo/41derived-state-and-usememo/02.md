# What Is Derived State?

#### Course: React Hooks: Level Up Your First Web App

## What Is Derived State?

Derived state is any value you can compute from existing state or props. Therefore, it doesn't need to be stored.

In the previous example, filteredEntries is not new data — it is entries passed through a filter. Given entries and a query, you can always recompute it. That makes it a derived state.

![Idea Emoji](data:image/png;base64,)

Storing a derived value creates two sources of truth where only one should exist. The moment they disagree, you have a bug.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


