# What Is a Custom Hook?

#### Course: React Hooks: Level Up Your First Web App

## What Is a Custom Hook?

A custom hook is a plain JavaScript function that:

- Has a name starting with use
- Can call other hooks inside it (useState(), useEffect(), useMemo(), etc.)
- Returns whatever the caller needs

That's the entire definition. No special syntax. No new APIs.

![Idea Emoji](data:image/png;base64,)

Next, let's look at a simple example of a custom hook.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


