# Lesson Summary

#### Course: React Hooks: Level Up Your First Web App

## Lesson Summary

Here's a summary of what you've learned in this lesson:

1. Encapsulation

usePagination() encapsulates the following inside it:

- useState() for currentPage
- useMemo() for totalPages and pagedItems
- handleNext() and handlePrev() handlers

2. Return Value

usePagination() returns the following object:

```react
{ page, pagedItems, totalPages, handleNext, handlePrev, reset }
```

![Idea Emoji](data:image/png;base64,)

3. Generic Nature

usePagination() is generic, i.e., it works with any array, not just journal entries.

In the next lesson, we'll look at the completed picture — the full mental model for composing hooks in React, and where to go from here.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


