# What You'll Build

#### Course: React Hooks: Level Up Your First Web App

## What You'll Build

The app you built in React Fundamentals was self-contained: entries lived in state, and a form added them.

In this course, you'll replace that with something closer to a real product:

| App Feature | Description |
| --- | --- |
| Real data from an API | Entries load from a server on startup, not from a form. |
| Loading and error states | The UI reacts while the request is in flight or fails. |
| A search bar | Entries are filtered in real time as you type. |
| Pagination | Entries are split across pages instead of all loading at once. |
| An entry detail drawer | Click any entry to open a full-detail panel alongside the list. |

Each feature introduces one new hook. By the end, you'll have done the following:

1. Utilize useEffect(), useRef(), useMemo(), and useCallback().
2. Extract all repeated logic into your own custom hooks.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


