...
There's two things to notice in every useEffect() call:

- function — This is your side effect, i.e., it's the code that runs after rendering.
- [] — An empty array. This tells React: "run this effect once, when the component first appears on screen". You'll learn exactly what this controls later. For now, just know it prevents the infinite loop.
...
Next, let's utilize useEffect() to escape from the infinite loop we encountered earlier.
