# Revisiting the Changes

#### Course: React Hooks: Level Up Your First Web App

## Revisiting the Changes

Here's what changed from the previous lesson:

- App now has two new state variables: isLoading (starts as true) and error (starts as null).
- The fetch is wrapped in try...catch.
- setIsLoading(false) runs in both try and catch branches since the loading always ends either way.
- A res.ok check throws manually for bad HTTP status codes, routing them to the catch block.
- The render now handles four possible states: loading, error, empty list, and data.

![Idea Emoji](data:image/png;base64,)

Next, let's explore one particular behavior of handling loading errors that trips up a lot of developers.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


