# Summary: Misconceptions and Best Practices

#### Course: React Hooks: Level Up Your First Web App

## Summary: Misconceptions and Best Practices

So there was my story of how I botched my dashboard. Here's the most important takeaways from that episode:

The Misconception

[] means "run once and you're done." That's correct for any effect that only needs to happen at startup.

The Truth

[] means "this effect uses no values from the component." If you reference a value and leave it out, the effect silently operates on a stale snapshot.

The Golden Rule

Every state variable, prop, and context value the effect reads belongs in the dependency array.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


