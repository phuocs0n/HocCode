# Installing Claude Code

#### Course: Rapid Build With Claude Code

## Installing Claude Code

Claude Code works in two environments: the terminal and VS Code. Install it for whichever you plan to use — or both.

Terminal

Install Claude Code globally with npm:

```react
npm install -g @anthropic-ai/claude-code
```

Once installed, authenticate with your Anthropic account:

```react
claude auth
```

This opens a browser window to complete sign-in. After authenticating, run a quick check to confirm everything is working:

```react
claude --version
```

If you see a version number, Claude Code is ready.

VS Code Extension

Open VS Code, go to the Extensions panel (Cmd+Shift+X / Ctrl+Shift+X), and search for Claude Code. Install the extension published by Anthropic. Once installed, a Claude icon appears in the Activity Bar on the left. Click it to open the chat panel and sign in with your Anthropic account.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


