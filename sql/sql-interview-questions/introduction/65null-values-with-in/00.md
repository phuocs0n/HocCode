# What will happen if we execute the following query?

#### Course: SQL Interview Questions

## What will happen if we execute the following query?

```sql
1SELECT *
2FROM Employees
3WHERE department IN ('Sales', 'Development', NULL);
```

The query may not return any results because comparing with NULL using the IN operator will result in unknown NULL values.

To include NULL values, you can use the IS NULL operator along with an OR operator:

```sql
1SELECT *
2FROM Employees
3WHERE department IN ('Sales', 'Development')
4  OR department IS NULL;
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


