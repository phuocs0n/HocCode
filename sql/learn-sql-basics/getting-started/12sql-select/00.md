# Introduction to SQL SELECT

#### Course: Learn SQL Basics

## Introduction to SQL SELECT

As an analyst, your first step is to explore the database and understand what you're working with. Your manager will have questions about the data, so let's start by getting a clear view of it.

SQL provides the SELECT statement to retrieve data from a database. This is the foundation for everything you'll do with SQL.

In this lesson, you'll use SELECT to pull information from the Customers table.

![Idea Emoji](data:image/png;base64,)

Now, let's write your first SQL query and see how SELECT works.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


