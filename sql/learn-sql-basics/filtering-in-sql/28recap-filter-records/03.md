# Narrowing Down Using WHERE

#### Course: Learn SQL Basics

## Narrowing Down Using WHERE

To identify bonus-eligible employees, start by filtering employees in the Sales department:

```sql
1-- Filter employees in Sales
2
3SELECT *
4FROM Employees
5WHERE department = 'Sales';
```

The WHERE clause filters rows to include only those that meet the specified condition.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


