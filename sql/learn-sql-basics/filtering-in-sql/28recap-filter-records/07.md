# Handling Missing Data and Patterns

#### Course: Learn SQL Basics

## Handling Missing Data and Patterns

Sometimes, you need to search using incomplete or partial data. For example, you need to find employees who are missing last_name and whose first_name ends in 'e'.

1. Filter for Missing Data:

```sql
1-- Find employees with missing last names
2
3SELECT *
4FROM Employees
5WHERE last_name IS NULL;
```

Unlike known values, you cannot use comparison operators when working with NULL. Instead, use IS NULL or IS NOT NULL.

2. Add a pattern match for first_name:

```sql
1-- Find employees with specific name patterns
2
3SELECT *
4FROM Employees
5WHERE last_name IS NULL
6AND first_name LIKE '%e';
```

The LIKE clause, combined with wildcards (% for zero or more characters, _ for a single character), allows you to search for patterns in text.

![Idea Emoji](data:image/png;base64,)

You can apply these filtering techniques to other tables and scenarios whenever necessary.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


