# Recap

#### Course: Learn SQL Basics

## Recap

Congratulations on completing this chapter!

In this chapter, you've worked on filtering data to retrieve relevant data. Knowing how to retrieve what you need is essential for making informed decisions as a data analyst.

Let's take a moment to review what you've learned so far.

Imagine you're managing bonuses and employee data for your startup using the Employees table.

Employees

| id | first_name | last_name | department | age |
| --- | --- | --- | --- | --- |
| 1 | Alice |  | Operations | 25 |
| 2 | Bruce | Banner | Operations | 26 |
| 3 | Clancy | Cooper | Finance | 27 |
| 4 | Dan | Davies | Sales | 25 |
| 5 | Eve |  | Operations | 24 |
| 7 | George | Gates | Sales | 19 |
| 8 | Hillary | Holmes | Marketing | 26 |

You might notice some empty cells in the table. These represent missing values, also known as NULLs, which are commonly found in datasets.

![Idea Emoji](data:image/png;base64,)

Let's revisit SQL filtering and see how they can help you retrieve the required data from this table.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


