# Examples: LIKE

#### Course: Learn SQL Basics

## Examples: LIKE

Let's see how you can use the LIKE operator to identify patterns in customer data and gather insights for a marketing campaign.

Example 1: Customers with First Names Containing 'o'

```sql
1-- Select customers whose first name 
2-- contains 'o'
3
4SELECT *
5FROM Customers
6WHERE first_name LIKE '%o%';
```

The above query matches John and Robert.

Example 2: Customers from the USA whose last name ends with 'a'

```sql
1-- Select customers from the USA whose last
2-- name ends with 'a'
3
4SELECT *
5FROM Customers
6WHERE country = 'USA' AND last_name LIKE '%a';
```

The above query returns the customer Robert.

Example 3: Customers Whose First Name Starts with 'J' and Ends with 'n'

```sql
1-- Select customers whose first name starts
2-- with 'J' and ends with 'n'
3
4SELECT *
5FROM Customers
6WHERE first_name LIKE 'J%n';
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


