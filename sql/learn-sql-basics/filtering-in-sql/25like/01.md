# SQL LIKE

#### Course: Learn SQL Basics

## SQL LIKE

In the previous lessons, we used the = operator to filter rows. For example,

```sql
1-- Select customers whose last name is 
2-- Robinson
3
4SELECT *
5FROM Customers
6WHERE last_name = 'Robinson';
```

However, instead of filtering rows based on a specific last name, what if you want to filter rows for customers whose last name starts with the letter 'R'?

In such cases, you can use the LIKE operator to select rows that match a specified pattern in a column. For example,

```sql
1-- Select customers whose last name starts
2-- with the letter 'R'
3
4SELECT *
5FROM Customers
6WHERE last_name LIKE 'R%';
```

Here, 'R%' indicates any string that begins with 'R'. The % wildcard indicates that 'R' can be followed by zero or more characters.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


