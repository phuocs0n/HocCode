# Chapter Overview

#### Course: Learn SQL Basics

## Chapter Overview

In this chapter, you'll learn about the following filtering tools in SQL:

- WHERE
- AND, OR, and NOT
- IN and BETWEEN
- LIKE and Wildcards
- LIMIT

![Idea Emoji](data:image/png;base64,)

We'll start with the most basic filtering tool: the WHERE clause. Let's see how it works.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


