# SQL INNER JOIN

#### Course: Learn SQL Basics

## SQL INNER JOIN

An INNER JOIN combines records from two tables where the specified condition is met, excluding unmatched rows from both tables.

Let's see how you can use an INNER JOIN to combine data from the Customers and Orders tables.

```sql
1-- Inner Join the Customers and Orders table
2
3SELECT *
4FROM Customers
5INNER JOIN Orders
6ON Customers.customer_id = Orders.customer_id;
```

In this query, the Customers and Orders tables are joined on the customer_id column. This means only rows with matching customer_id values in both tables are included in the result.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


