# SQL LEFT JOIN

#### Course: Learn SQL Basics

## SQL LEFT JOIN

A LEFT JOIN joins two or more tables and selects all the rows from the left table and matching rows from the right table.

Let's see how you can use a LEFT JOIN to combine data from the Customers and Orders tables.

```sql
1-- Join the Customers and Orders table using LEFT JOIN
2
3SELECT *
4FROM Customers
5LEFT JOIN Orders
6ON Customers.customer_id = Orders.customer_id;
```

Here, the LEFT JOIN ensures that all rows from the Customers table are included. If there is no matching customer_id in Orders, the result shows NULL for the Orders' columns.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


