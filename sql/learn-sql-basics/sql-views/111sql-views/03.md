# Creating a View

#### Course: Learn SQL Basics

## Creating a View

Let's create a view that stores only the age and country of customers.

```sql
1-- Create a view named customer_demographics
2
3CREATE VIEW customer_demographics AS
4SELECT age, country
5FROM Customers;
```

When you run the code, it creates a view named customer_demographics.

Here,

- Customers - actual physical table (also called base table),
- customer_demographics - virtual table (also called view).

However, you won't see any output immediately because the view does not display data by itself.

To see the results, you need to execute a SELECT statement on the customer_demographics view.

```sql
1-- Select all the rows from customer_demographics
2
3SELECT * 
4FROM customer_demographics
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


