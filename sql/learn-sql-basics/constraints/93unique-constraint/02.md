# UNIQUE Constraint

#### Course: Learn SQL Basics

## UNIQUE Constraint

Now, let's address the second requirement: No two books can have the same book code.

To meet this requirement, you can set up a title column with the NOT NULL constraint when creating the Books table.

The UNIQUE constraint in SQL is a rule that ensures all values in a column are distinct (no duplicate value allowed).

Here's how you can implement this:

```sql
1CREATE TABLE Books (
2    title VARCHAR(100) NOT NULL,
3    book_code VARCHAR(20) UNIQUE,
4    publisher VARCHAR(100)
5);
```

When you run the above query, a table is created with a book_code column with a unique constraint.

What happens if you try to insert two books with the same book code? Let's find out next.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


