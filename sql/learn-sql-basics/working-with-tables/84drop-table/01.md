# The DROP TABLE Statement

#### Course: Learn SQL Basics

## The DROP TABLE Statement

To remove a table permanently from the database, you use the DROP TABLE statement. It deletes both the tables and all its content.

For example, your database currently contains two tables:

- Customers
- Products

Your manager informs you that the Customers table is no longer needed because customer data is now managed in a separate system. You can delete it using:

```sql
1DROP TABLE Customers;
```

Before, our database had two tables: Customers and Products. After you run the code, the Customers table will be deleted.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


