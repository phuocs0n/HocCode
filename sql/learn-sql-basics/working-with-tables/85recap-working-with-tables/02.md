# CREATING TABLE

#### Course: Learn SQL Basics

## CREATING TABLE

In order to create a proper system to store customer details, you can start by creating a table in SQL to organize customer information properly.

```sql
CREATE TABLE Customers (
    id INT,
    first_name VARCHAR(50),
    last_name VARCHAR(50),
    age INT,
    country VARCHAR(50)
);
```

This creates a new table with defined columns and data types.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


