# Aggregate Window Functions

#### Course: Learn SQL Basics

## Aggregate Window Functions

Your manager is reviewing sales data and asks:

"Can we track the days with the highest and lowest sales amounts? Also, how many transactions have we processed so far?"

To answer this, you can use window functions with MIN(), MAX(), and COUNT() to analyze sales trends while keeping all the daily records intact.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


