# Example: Window Function for Average Salary

#### Course: Learn SQL Basics

## Example: Window Function for Average Salary

Let's modify our query to use a window function:

```sql
1SELECT *, AVG(salary) OVER()
2AS average_salary
3FROM Employees;
```

Now, you have the average salary for each employee's row. This allows you to compare each employee's salary with the company average.

Notice the use of OVER()---this defines a window function that calculates the average salary without collapsing the rows into a single result.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


