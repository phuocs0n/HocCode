# Updating Existing Book Information

#### Course: Learn SQL Basics

## Updating Existing Book Information

The team then raises an important issue: "There's an error in the database! The book Pride and Prejudice is mistakenly categorized under the genre Action. How can we correct this and update its genre to Romance?"

For this, you can use the UPDATE statement to modify the existing record:

```sql
1UPDATE Books
2SET genre = 'Romance'
3WHERE title = 'Pride and Prejudice';
```

The above query changes the Pride and Prejudice book genre from Action to Romance.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


