# Deleting All the Data

#### Course: Learn SQL Basics

## Deleting All the Data

What happens if you don't include the WHERE clause in a DELETE FROM statement? It will remove all records from the table.

Let's write a query to delete records from the Students table without using a WHERE clause.

```sql
1-- Delete all the data
2
3DELETE FROM Students;
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


