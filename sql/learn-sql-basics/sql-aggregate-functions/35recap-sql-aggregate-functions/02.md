# Finding Price Range

#### Course: Learn SQL Basics

## Finding Price Range

Understanding the price range helps you evaluate how diverse your product pricing is. It can show if you cater to different budgets or if there are gaps in your offerings.

Use the MIN() and MAX() functions to find the lowest and highest prices. You can calculate both in a single query:

```sql
1SELECT MIN(price) AS min_price, MAX(price) AS max_price
2FROM Products;
```

![Idea Emoji](data:image/png;base64,)

Insights:

- Lowest price: $50 (mousepad)
- Highest price: $250 (keyboard)

This information can guide pricing strategies and promotional decisions.

Let's move on to the next insight!

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


