# Example: if Statement

#### Course: Learn C++ Basics

## Example: if Statement

```cpp
1// check whether a student passed or failed the exam.
2// we will assume the passing grade is 50 or more.
3
4#include <iostream>
5using namespace std;
6
7int main() {
8
9    int student_grade;
10  
11    // get student score from the user
12    cout << "Enter grade: ";
13    cin >> student_grade;
14
15    // if grade is 50 or above, student_grade >= results to true
16    if (student_grade >= 50) {
17        cout << "Congratulations!" << endl;
18        cout << "You passed the examination.";
19    }
20  
21    return 0;
22}
```

Sample Output 1

```text
Enter grade: 72
Congratulations!
You passed the examination.
```

If the user enters 72, student_grade >= 50 in the if statement will be true. In this case, the statements inside the if block are executed.

Sample Output 2

```text
Enter grade: 42
```

If the user enters 42, student_grade >= 50 in the if statement will be false. In this case, the statements inside the if block are skipped from execution.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


