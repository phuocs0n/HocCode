# Important!

#### Course: Learn C++ Basics

## Important!

The lessons we learned in this chapter are used to build programming logic. And you can only become better at it by practicing.

If you think everything is not quite clicking for you, please revise the chapter and write all the programs again before proceeding further. This will help you in the long run. :)

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


