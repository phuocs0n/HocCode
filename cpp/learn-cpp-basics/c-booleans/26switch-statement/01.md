# switch Statement

#### Course: Learn C++ Basics

## switch Statement

Here's the syntax of a switch statement.

```cpp
switch (variable/expression) {

    case value1:
        // body of case 1
        break;

    case value2:
        // body of case 2
        break;

    case valueN:
        // body of case N
        break;

    default:
        // body of default
}
```

The switch statement starts with the switch keyword, followed by a variable or an expression inside the parentheses.

Inside the switch statement, we have multiple cases to perform multiple operations.

Here's how it works.

If the value/expression matches with

- value1 - body of the first case is executed
- value2 - body of the second case is executed
- valueN - body of the nth case is executed

If the value/expression doesn't match the value of any case, the default body is executed.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


