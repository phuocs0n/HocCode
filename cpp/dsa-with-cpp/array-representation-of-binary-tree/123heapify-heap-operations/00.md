# Heapify

#### Course: DSA with C++

## Heapify

Heapify is the process of rearranging nodes in a binary tree such that every node satisfies the heap property.

There are two types of heapify operations:

#### Heapify Up

Heapfiy up moves up the tree (from leaf to root) while comparing to the current parent element and swapping if needed.

It's used to restore the heap property after inserting a new element to the heap.

#### Heapify Down

Heapify down moves down the tree, comparing the root to its children and swapping if needed.

It's used to restore the heap property after we have removed the root node from the heap.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


