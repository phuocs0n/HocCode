# Example: Lambda Capturing Variables

#### Course: Learn C++ STL

## Example: Lambda Capturing Variables

```cpp
1#include <iostream>
2
3using namespace std;
4
5int main(){
6    int i = 5;
7    auto val = [=](int x, int y) -> int {
8        return i + x + y;
9    } (5,5);
10
11
12    cout << "The sum is : " << val;
13}
```

Output:

```text
The sum is : 15
```

Here, note the [=] expression, which indicates that we are capturing the variables in surrounding scope by value.

This [=] expression allows us to use variables in the surrounding scope within the lambda body.

We can then reference the variable i within the lambda body.

![Idea Emoji](data:image/png;base64,)

We can also capture the surrounding variables by reference using [&], which we'll later.

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


