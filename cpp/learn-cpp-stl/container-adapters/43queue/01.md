# Create a Queue

#### Course: Learn C++ STL

## Create a Queue

The queue class is defined inside the queue header file. So, we first have to import this file to utilize the STL queue.

```cpp
#include <queue>
```

Once we import the file, we can create a list using the queue keyword.

```cpp
queue<int> numbers;
```

Here, we have created an integer queue named numbers. Now, we can use this list to store the numbers.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


