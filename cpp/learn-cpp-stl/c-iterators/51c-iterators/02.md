# Example: Dereferencing Iterator Pointer to Access Value

#### Course: Learn C++ STL

## Example: Dereferencing Iterator Pointer to Access Value

```cpp
1#include <iostream>
2#include <vector>
3
4using namespace std;
5
6int main() {
7    vector<int> numbers = {12, 2 ,3, 4, 5};
8    
9    auto iter = numbers.begin();
10
11    // dereferencing the iterator to access the element
12    cout << *iter << endl;
13
14    return 0;
15}
```

Output

```text
12
```

This just shows how similar iterators are to pointers.

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


