# Example: accumulate()

#### Course: Learn C++ STL

## Example: accumulate()

Consider we have a vector of integers,

```cpp
vector<int> numbers {1, 2, 3, 4, 5};
```

To compute its sum,

```cpp
accumulate (numbers.begin(), numbers.end() , 0);
```

Here, 0 is an int ,so the function will return 0. Let's see a complete example.

```cpp
1#include <iostream>
2#include <numeric>
3#include <vector>
4
5using namespace std;
6
7int main(){
8
9    vector<int> numbers{1, 2, 3, 4, 5};
10
11   // Accumulate from the beginning of the container to the end with the initial value of 0
12    int sum = accumulate(numbers.begin(), numbers.end(), 0);
13
14    cout << "The sum is " << sum;
15}
```

Output:

```text
The sum is 15
```

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


