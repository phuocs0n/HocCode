# C++ Lambda Expressions

#### Course: Learn C++ Intermediate

## C++ Lambda Expressions

Now, let's move on to the concept of lambda expressions, which was introduced in C++ 11.

In C++, lambda expressions are anonymous inline functions i.e. they

- don't have any names (anonymous)
- are used and defined inside other expressions or passed as arguments (inline)

For example, consider a regular function:

```cpp
void print() {
    cout << "Hello World!" << endl;
}
```

Its lambda equivalent will be:

```cpp
[]() {
    cout << "Hello World!" << endl;
}
```

Here,

- [] is called the lambda introducer which denotes the start of the lambda expression
- () is called the parameter list which is similar to the () operator of a normal function

![Idea Emoji](data:image/png;base64,)

##### How does it work?

Use this space for your doubts about this lesson. Ask us anything that's confusing and our experts will provide explanations to help you understand.

## 📝 Quiz

****


